...
// --- DEĞİŞKENLER ---
let isTouchActive = false; // Tablette mouse'u engellemek için kilit
let isDrawing = false; 
...
const closePdfButton = document.getElementById('btn-close-pdf');
...
function clearAllStrokes() {
    ...
    // DEĞİŞİKLİK BURADA: Sadece 'image' OLMAYANLARI temizle.
    // Yani tipi 'image' olanlar (kitap/resim) hafızada kalır.
    drawnStrokes = drawnStrokes.filter(stroke => stroke.type === 'image');
    
    window.drawnStrokes = drawnStrokes; 
    nextPointChar = 'A';
    window.nextPointChar = 'A';
    
    // Not: backgroundImage değişkenini null yapmıyoruz artık.
...
function addToCanvasAsObject(img) {
    // Resmi ekrana sığacak makul bir boyuta getir
    let startWidth = 400; // PDF'ler genelde büyük olur, 400 iyi bir başlangıç
    
    // Eğer resim çok küçükse (ikon vs) kendi boyutunu koru
    if (img.width < 400) startWidth = img.width;
    
    let scaleFactor = startWidth / img.width;
    let startHeight = img.height * scaleFactor;

    // Resmi Hafızaya "Nesne" Olarak Ekle
    drawnStrokes.push({
        type: 'image',
        img: img, 
...
// --- app.js içindeki canvas.addEventListener('mousedown', ...) bloğunu TAMAMEN bununla değiştir ---
...
canvas.addEventListener('mousedown', (e) => {
    ...
    // --- YENİ EKLENEN KISIM: MENÜLERİ GİZLE ---
    // Çizim alanına (boşluğa) tıklandığında açık olan tüm menüleri kapat
   ...
   if (currentTool === 'move') { // Taşıma Mantığı
        ...
        if (hit) { // Bir şey yakalandı
            ...
            else if (hit.pointKey === 'rotate' || hit.pointKey === 'resize') originalStartPos = { radius: hit.item.radius, rotation: hit.item.rotation };
            ...
            return; 
             ...
             // Hiçbir şeye tıklanmadıysa, seçimi iptal et
             if (selectedItem) {
...
canvas.addEventListener('mousemove', (e) => {
    ...
    // 'move' modu mantığı 'mousemove' İÇİNE TAŞINDI
    if (currentTool === 'move' && isMoving) {
        const pos = getEventPosition(e);
        const dx = pos.x - dragStartPos.x;
        const dy = pos.y - dragStartPos.y;
        
        // 1. RESİM BOYUTLANDIRMA (YENİ EKLENEN KISIM)
        if (selectedPointKey === 'image_resize') {
            // Merkezden fareye olan mesafeyi hesapla
            const distFromCenterX = Math.abs(pos.x - selectedItem.x);
            const distFromCenterY = Math.abs(pos.y - selectedItem.y);
            
            // Genişlik/Yükseklik = Merkezden uzaklığın 2 katı
            // (Min 20px koruması ekledik ki resim kaybolmasın)
        ...
        // 5. GENEL TAŞIMA (Resim Gövdesi, Çokgen Merkezi vb.)
        else {
            if (selectedPointKey === 'self') { // Resim veya Nokta gövdesi
    ...
    if (currentTool === 'straightLine' && isDrawingLine) {
        ...
        redrawAllStrokes(); 
        const endPos = snapTarget || currentMousePos; // Yakalama
        ctx.beginPath();
        ctx.moveTo(lineStartPoint.x, lineStartPoint.y);
        ctx.lineTo(endPos.x, endPos.y);
        ctx.strokeStyle = currentLineColor;
        ...
        redrawAllStrokes(); 
        drawInfinityLine(lineStartPoint, currentMousePos, currentLineColor, 3, true); // true = Işın
        drawDot(lineStartPoint, currentLineColor);
        previewActive = true;
    }
        ...
        if (type === 0) { // Çember önizlemesi
            ctx.arc(center.x, center.y, currentRadius, 0, 2 * Math.PI);
        } else { // Düzgün Çokgen önizlemesi
        ...
        if (type === 0) { // Çember
            labelText = `Yarıçap: ${cmRadius} cm`;
        } else { // Çokgen
            const s_px = 2 * currentRadius * Math.sin(Math.PI / type);
            const s_cm = (s_px / (window.PolygonTool.PIXELS_PER_CM || 30)).toFixed(1);
            labelText = `Kenar: ${s_cm} cm`;
        }
    ...
    ctx.globalAlpha = 1.0;
    ctx.setLineDash([]);
    if (previewActive) return; // Ön izleme yapıldıysa, kalem/silgiye geçme


    // --- Kalem/Silgi Sürükleme Mantığı ---
    ...
    if (currentTool === 'pen') {
        ...
        for (const stroke of drawnStrokes) {
            ...
            if (stroke.type === 'pen') {
                for (const point of stroke.path) {
                    if (distance(point, pos) < 10) { touched = true; break; }
                }
            } else if (stroke.type === 'point') {
                ...
                for (let i = 0; i <= steps; i++) {
                    ...
                    if (distance({x: sampleX, y: sampleY}, pos) < 10) {
                        touched = true;
                        break;
                    }
                }
            ...
            // --- DÜZELTME: DOĞRU PERGEL SİLGİSİ (360 Derece Uyumlu) ---
            else if (stroke.type === 'arc') {
                
                // --- YENİ KOD: MERKEZİ KONTROL ET ---
                const centerPos = { x: stroke.cx, y: stroke.cy };
                if (distance(centerPos, pos) < 10) { // pos = touch
                    touched = true;
                }
                // --- YENİ KOD SONU ---

                // YAYI KONTROL ET (Eğer merkeze dokunmadıysak)
                if (!touched) {
                    const PI_RAD = Math.PI / 180;
                    const steps = 30; 
                    let start = stroke.startAngle;
                    let end = stroke.endAngle;
                    const totalAngle = end - start;
                    
                    for (let i = 0; i <= steps; i++) {
                        const t = i / steps;
                        const angle = (start + t * totalAngle) * PI_RAD;
                        
                        // --- KRİTİK YAZIM HATASI DÜZELTMESİ ---
                        const sampleX = stroke.cx + stroke.radius * Math.cos(angle); // <-- HATA DÜZELTİLDİ
            ...
            // --- YENİ EKLENEN BLOK (ÇOKGEN SİLGİSİ) ---
            else if (stroke.type === 'polygon') {
                
                // 1. Merkezi kontrol et
                if (stroke.center && distance(stroke.center, pos) < 10) {
                    touched = true;
                }

                // 2. Köşeleri (vertices) kontrol et (Eğer merkez bulunamadıysa)
                if (!touched && stroke.vertices) {
                    for (const v of stroke.vertices) {
                        if (distance(v, pos) < 10) {
                            touched = true;
                            break;
                        }
                    }
                ...
                // 3. Kenarları (edges) kontrol et (Eğer hala bulunamadıysa)
                if (!touched && stroke.vertices) {
                    for (let j = 0; j < stroke.vertices.length; j++) {
                        // Köşeleri al
                        const v1 = stroke.vertices[j];
                        ...
                        const lineLength = distance(v1, v2);
                        // Çizgi boyunca 5px'lik adımlarla kontrol et
                        const steps = Math.max(1, Math.floor(lineLength / 5)); 
                        
                        for (let i = 0; i <= steps; i++) {
                            const t = i / steps;
                            // Kenar üzerindeki örnek noktayı hesapla
                            const sampleX = v1.x + (v2.x - v1.x) * t;
                            const sampleY = v1.y + (v2.y - v1.y) * t;
                            
                            // Silgiye (pos) yakın mı?
                            if (distance({x: sampleX, y: sampleY}, pos) < 10) {
                                touched = true;
                                break;
                            }
                        }
                        if (touched) break; // Kenar bulundu, dış döngüden çık
                    }
                }
            }
            // --- YENİ BLOK SONU ---
        ...
        if (needsRedraw) {
            ...
            window.drawnStrokes = strokesToKeep; // <-- HAFIZA DÜZELTMESİ
            redrawAllStrokes(); 
        }
    } // else if (currentTool === 'eraser') sonu
});
...
canvas.addEventListener('mouseup', () => {
    ...
    if (currentTool === 'move' && isMoving) {
        ...
        // --- KRİTİK DÜZELTME ---
        // Sadece 'eylemi' bitir, 'seçimi' değil.
        isMoving = false;
        // selectedItem = null; // <-- HATA BUYDU, BU SATIR SİLİNDİ!
...
// --- DOKUNMATİK BAŞLANGIÇ (TOUCHSTART - GÜNCELLENMİŞ) ---
canvas.addEventListener('touchstart', (e) => {
    isTouchActive = true; // Dokunmatik moda geç, mouse'u kilitle
    ...
    // Koordinatı al ve global değişkene ata
    const pos = getEventPosition(e);
    currentMousePos = pos; 
    
    // 1. Fiziksel Araç Kontrolü (Cetvel vb. üzerindeyse çizimi başlatma)
    // Eğer parmak bir aracın üzerindeyse, canvas çizimini engelle
    const touch = e.touches[0];
    const target = document.elementFromPoint(touch.clientX, touch.clientY);
    if (target && target.closest('.ruler-container, .gonye-container, .aciolcer-container, #compass-container')) {
        return; 
    }


    // 1. Taşıma (Move) ve Bilgi Gösterme
    if (currentTool === 'move') {

if (e.touches.length === 2 && currentTool === 'move' && selectedItem && selectedItem.type === 'image') {
    // İki parmak algılandı ve bir resim/pdf seçili
    isPinching = true;
    isMoving = false; // Tekil taşımayı durdur
    ...
    // Taşıma pivotunu (merkez) pinch başlangıcına ayarla
    dragStartPos = initialCenter; 
    originalStartPos = { x: selectedItem.x, y: selectedItem.y }; 

    return; 
}
// Eğer 1 parmak varsa, normal taşıma başlat (isMoving = true)
else if (e.touches.length === 1 && currentTool === 'move') {
    // Normal tek parmak taşıma/kaydırma mantığı devam eder.
    // ... (Mevcut isMoving mantığı)
}

        const hit = findHit(pos);
        if (hit) {
            // Bilgi Etiketlerini Aç/Kapat (Dokunarak)
            if (hit.pointKey === 'toggle_edges') { hit.item.showEdgeLabels = !hit.item.showEdgeLabels; redrawAllStrokes(); return; }
            if (hit.pointKey === 'toggle_angles') { hit.item.showAngleLabels = !hit.item.showAngleLabels; redrawAllStrokes(); return; }
            if (hit.pointKey === 'toggle_circle_info') { hit.item.showCircleInfo = !hit.item.showCircleInfo; redrawAllStrokes(); return; }

            
            isMoving = true; selectedItem = hit.item; selectedPointKey = hit.pointKey; dragStartPos = pos; rotationPivot = null; 
...
if (isPinching) {
    ...
    const newHeight = selectedItem.height * (newWidth / selectedItem.width); // Oranı koru

    // 2. Pan (Kaydırma/Taşıma)
    // Zoom merkezini sabit tutacak şekilde pivotu ayarla (pinchingCenter)
    ...
    // Basit kaydırma (Pivotu sabit tutma denklemi karmaşıktır, basit kaydırma uyguluyoruz)
...
// Eğer isPinching false ise ve isMoving true ise, normal taşıma mantığı devam eder.
if (currentTool === 'move' && isMoving) {
    // ... (Mevcut tek parmak taşıma/kaydırma mantığı devam eder)
}
    if (currentTool === 'ruler' || currentTool === 'gonye' || currentTool === 'aciolcer' || currentTool === 'pergel') return;
    e.preventDefault(); 
    if (currentTool === 'none') return;
    ...
    if (canSnap) {
        snapTarget = findSnapPoint(pos);
        if (snapTarget) { snapIndicator.style.left = `${snapTarget.x}px`; snapIndicator.style.top = `${snapTarget.y}px`; snapIndicator.style.display = 'block'; } 
        else { snapIndicator.style.display = 'none'; }
    } else { snapTarget = null; snapIndicator.style.display = 'none'; }


    // --- ÖN İZLEMELER (KESİKLİ ÇİZGİ) ---
    let previewActive = false;
    ctx.globalAlpha = 0.6; 
    ctx.setLineDash([10, 5]); // <-- KESİKLİ ÇİZGİ AYARI

    const endPos = snapTarget || currentMousePos;

    if (currentTool === 'straightLine' && isDrawingLine) {
        redrawAllStrokes(); ctx.beginPath();
        ctx.moveTo(lineStartPoint.x, lineStartPoint.y); ctx.lineTo(endPos.x, endPos.y);
        ctx.strokeStyle = currentLineColor; ctx.lineWidth = 3; ctx.stroke();
        drawDot(lineStartPoint, currentLineColor); drawDot(endPos, currentLineColor);
        previewActive = true; 
    }
    else if (currentTool === 'line' && isDrawingInfinityLine) {
        redrawAllStrokes();
        drawInfinityLine(lineStartPoint, currentMousePos, currentLineColor, 3, false);
        drawDot(lineStartPoint, currentLineColor); drawDot(currentMousePos, currentLineColor);
        previewActive = true;
    }
    else if (currentTool === 'segment' && isDrawingSegment) {
        redrawAllStrokes(); 
        ctx.beginPath(); ctx.moveTo(lineStartPoint.x, lineStartPoint.y); ctx.lineTo(endPos.x, endPos.y);
        ctx.strokeStyle = currentLineColor; ctx.lineWidth = 3; ctx.stroke();
        drawDot(lineStartPoint, currentLineColor); drawDot(endPos, currentLineColor);
        previewActive = true;
    }
        ...
        const type = window.tempPolygonData.type;
        const currentRadius = distance(center, currentMousePos);
        const dx = currentMousePos.x - center.x; const dy = currentMousePos.y - center.y;
        const currentRotationRad = Math.atan2(dy, dx); 
        const currentRotationDeg = currentRotationRad * (180 / Math.PI); 
        ...
        polygonPreviewLabel.style.left = `${currentMousePos.x}px`;
        polygonPreviewLabel.style.top = `${currentMousePos.y - 50}px`;
        polygonPreviewLabel.classList.remove('hidden');
        const cmRadius = (currentRadius / (window.PolygonTool.PIXELS_PER_CM || 30)).toFixed(1);
    ...
    if (currentTool === 'pen') {
        ...
        for (const stroke of drawnStrokes) {
            let touched = false;
            if (stroke.type === 'pen') { for (const point of stroke.path) { if (distance(point, pos) < 10) { touched = true; break; } } }
                 ...
                 for(let i=0; i<=steps; i++) {
                    const t=i/steps; 
                    if(distance({x:p1.x+(p2.x-p1.x)*t, y:p1.y+(p2.y-p1.y)*t}, pos)<10) { touched=true; break; }
                 }
            } else if (stroke.type === 'arc') {
                 if(distance({x:stroke.cx, y:stroke.cy}, pos)<10) touched=true;
                 else {
                    const steps=30;
                    for(let i=0; i<=steps; i++) {
                        const angle = (stroke.startAngle + (i/steps)*(stroke.endAngle-stroke.startAngle)) * (Math.PI/180);
                        if(distance({x:stroke.cx+stroke.radius*Math.cos(angle), y:stroke.cy+stroke.radius*Math.sin(angle)}, pos)<10) { touched=true; break; }
                    }
                 }
            } else if (stroke.type === 'polygon') {
                 if(stroke.center && distance(stroke.center, pos)<10) touched=true;
                    ...
                    if(!touched) {
                        for(let j=0; j<stroke.vertices.length; j++) {
                            const v1=stroke.vertices[j]; const v2=stroke.vertices[(j+1)%stroke.vertices.length];
...
// app.js içindeki 'addNewImageToCanvas' fonksiyonunu bul ve sonuna şunu ekle:

function addNewImageToCanvas(img, isPDF = false) {
    // ... (Mevcut boyutlandırma kodların) ...
    let startWidth = 400; 
    if (img.width < 400) startWidth = img.width;
    let scaleFactor = startWidth / img.width;
    let startHeight = img.height * scaleFactor;
    ...
    // PDF sayfasıysa eskisini sil (yerine yenisi gelsin)
    if (isPDF && pdfImageStroke) {
         drawnStrokes = drawnStrokes.filter(s => s !== pdfImageStroke);
    }
    ...
    // --- YENİ EKLENEN KISIM: KAPAT BUTONUNU GÖSTER ---
    if (closePdfButton) {
        closePdfButton.classList.remove('hidden');
    }
    // --------------------------------------------------
...
// --- PDF / RESİM KAPATMA BUTONU MANTIĞI ---
if (closePdfButton) {
    closePdfButton.addEventListener('click', () => {
        // 1. Hafızadaki "image" (resim/pdf) nesnelerini sil
        drawnStrokes = drawnStrokes.filter(stroke => stroke.type !== 'image');
        window.drawnStrokes = drawnStrokes;

        // 2. PDF değişkenlerini sıfırla
        currentPDF = null;
        pdfImageStroke = null;
